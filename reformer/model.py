...
class MatrixMultiply(nn.Module):
    def __init__(self, tensor, transpose = False, normalize = False):
        super().__init__()
        self.tensor = tensor # 입력텐
        self.transpose = transpose # 트랜스포즈 여부
        self.normalize = normalize # 노말라이즈 여부

    def forward(self, x):
        tensor = self.tensor
        if self.normalize: #  노말라이즈
            tensor = F.normalize(tensor, dim=-1)
        if self.transpose: # 트랜스포즈
            tensor = tensor.t()
        return x @ tensor
...
class ReforBertLM(nn.Module):
    ...
    def __init__(self,
        ...
        self.pos_emb = nn.Embedding(max_seq_len+1, self.embedding_dim)
        self.seg_emb = nn.Embedding(2, self.embedding_dim)
        ...
        self.projection_lm = nn.Sequential(
            nn.Linear(self.embedding_dim, emb_dim) if emb_dim != self.embedding_dim else Identity(),
            nn.Linear(self.embedding_dim, num_tokens) if not weight_tie_embedding else MatrixMultiply(self.enc_emb.weight, transpose=True, normalize=True)
        )
    ...
    def forward(self, inputs, segments):
        ...
        pos_mask = inputs.eq(0) # 패딩 아이디로 마스크 생성
        positions.masked_fill_(pos_mask, 0)

        # reforBERT 학습 Input  (bs, n_enc_seq, d_hidn)
        bert_inputs = self.enc_emb(inputs) + self.pos_emb(positions) + self.seg_emb(segments)
